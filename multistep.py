...
def plotOptions(results, title, ylabel, keys):
    ...
    #plt.ylim(ymin=0.025, ymax=0.032)
    
    plt.show()

def plotMSE(results, title):
    plt.gca().set_color_cycle(None)
    
    for i in results:
        plt.plot(results[i]['mseEvolution'][:DELAYS])
    
    plt.legend(results.keys(), loc='upper left')
    
    plt.title(title)
    plt.ylabel('MSE')
    plt.xlabel('Unknown steps')
...
if __name__ == '__main__':
    ...
    for configName in configs:
        ...
        modelPath = resultsDir + '/' + configName + '.h5'
        ...
        mseFileName = resultsDir + '/' + configName + '.mse'
        
        if os.path.isfile(mseFileName):
            ...
            for delay in xrange(DELAYS):
                print 'Computing MSE with', delay, 'unknown steps'
                mseEvolution.append(model.evaluate(test_x, test_y, batch_size=batch_size, verbose=verbose))
                print
                
                print 'Preparing the next test dataset'
                predictions = model.predict(test_x, batch_size=batch_size, verbose=verbose)
                print
                
                for r in xrange(test_x.shape[0] - 1):
                    for c in xrange(test_x.shape[1] - 1):
                        test_x[r, c] = test_x[r, c + 1]
                    
                    test_x[r, -1] = predictions[r]
                    test_y[r] = test_y[r + 1]
        
            handler = open(mseFileName, 'wb')
            pickle.dump(mseEvolution, handler)
            handler.close()
            
            results[configName]['mseEvolution'] = mseEvolution
    ...
    plotMSE(results, 'Multistep MSE evolution')
